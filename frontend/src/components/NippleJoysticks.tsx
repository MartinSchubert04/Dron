...
interface Props {
  onLeftMove:  (throttle: number, yaw:  number) => void;
  onRightMove: (pitch:    number, roll: number) => void;
}
...
export function NippleJoysticks({ onLeftMove, onRightMove }: Props) {
  ...
  return (
    <>
      {/* Left — vertically centered on the left side */}
      <div className="absolute left-0 top-1/2 -translate-y-1/2 z-20 px-5">
        <JoyZone
          label="THR / YAW"
          sub="▲▼ Throttle  ·  ◀▶ Yaw"
      ...
      {/* Right — vertically centered on the right side */}
      <div className="absolute right-0 top-1/2 -translate-y-1/2 z-20 px-5">
        <JoyZone
          label="PITCH / ROLL"
          sub="▲▼ Pitch  ·  ◀▶ Roll"
