...
function GearIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
         strokeWidth="1.8" strokeLinecap="round" className="w-5 h-5">
...
export default function App() {
  ...
  const handleTiltStart = useCallback((dir: -1 | 1) => drone.setCameraTilt(dir), [drone]);
  ...
  const closePanel = () => { if (panelOpen) setPanelOpen(false); };
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-black select-none">
      ...
      {/* ── Top bar ──────────────────────────────────────────────────── */}
      <div className="absolute top-0 inset-x-0 h-14 z-30 flex items-center justify-between
        ...
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full flex-shrink-0 ${DOT_CLASS[drone.connState]}`} />
          <span className="text-white font-bold text-sm tracking-[0.2em]">TURBODRONE</span>
      ...
      <ActionPanel
        ...
        onTakeoff={() => { drone.takeoff(); setPanelOpen(false); }}
        onLand={() =>    { drone.land();    setPanelOpen(false); }}
        onEstop={() =>   { drone.estop();   setPanelOpen(false); }}
